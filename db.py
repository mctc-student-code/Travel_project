...
#     # save these things to the database
#     pass
def save(restaurants, weather):
    #save method the restaurant that the user book-marked.
    # rest = Restaurant("name", "address", 1) 
    
    for name, address, rating in zip(restaurants[0], restaurants[1], restaurants[2]):
        print(name, address, rating)
        conn.execute('insert into travel_tip(name, address, rating) values (?, ?, ?)', (name, address, rating ))
